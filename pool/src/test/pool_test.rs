...
fn init_reserve() {
    ...
    let pool: LendingPoolClient<'_> = create_pool_contract(&env, &admin);
    let s_token =
        create_s_token_contract(&env, &pool.address, &underlying_token.address);
...
fn withdraw_interest_rate_less_than_one() {
    ...
    sut.pool
        .deposit(&user1, &token.address, &deposit_amount);
...
fn deposit() {
    ...
    for i in 0..10 {
        ...
        sut.pool
            .deposit(&user, &token.address, &deposit_amount);
...
fn borrow() {
    ...
    for r in sut.reserves.iter() {
        ...
        sut.pool
            .deposit(&lender, &r.token.address, &deposit_amount);
...
fn test_liquidate_error_good_position() {
    ...
    token.mint(&user, &1_000_000_000);
    sut.pool
        .deposit(&user, &token.address, &1_000_000_000);
...
fn test_liquidate_error_not_enough_collateral() {
    ...
    sut.pool
        .deposit(&borrower, &token1.address, &deposit);
...
fn test_liquidate() {
    ...
    sut.pool
        .deposit(&lender, &debt_asset.address, &deposit);
...
fn user_operation_should_update_ar_coeffs() {
    ...
    sut.pool
        .deposit(&lender, &debt_asset_1, &deposit_amount);
...
fn repay() {
    ...
    for r in sut.reserves.iter() {
        let pool_balance = r.token.balance(&r.s_token.address);
        sut.pool
            .deposit(&lender, &r.token.address, &lending_amount);
